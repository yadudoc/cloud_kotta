...
def generate_signed_url(app, filepath):
    ...
    url = app.config["s3.conn"].generate_url(3600, # expiry
                                             method='GET', 
                                             bucket="klab-jobs",
                                             key="uploads/amzn1.account.AEKWXVYINCBBNY5MPRMOYND6CWWA/python/pkl/{0}".format(filepath),
                                             headers=None,
                                             force_http=False, 
...
def post_upload_url():
    ...
    try:
        ...
        url = generate_signed_url(request.app, fpath)
